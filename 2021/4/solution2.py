...
if __name__ == "__main__":
    with open("AdventD4.txt") as f:
        lines = f.readlines()
    numbers_to_call = list(map(int,lines[0].rstrip("\n").split(",")))
    winning_number = -1
    unmarked_sum = -1
    boards = []
    called = []
    ...
    boards_won = np.zeros(len(boards))
    all_won = False
    for called_number in numbers_to_call:
        print("calling number",called_number)
        ...
        for called_board_pos in range(len(called)):
            if checkIfWinner(called[called_board_pos]):
                ...
                boards_won[called_board_pos] = 1
                if np.all(np.where(boards_won==1,True,False)):
                    cached_winner = [winning_number,unmarked_sum]
                    all_won = True
                    break
        # check boards
        if all_won:
